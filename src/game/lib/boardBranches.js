...
export const hasOverlappingBranch = (
  branches,
  startId,
  endId,
  ignoreId = null,
...
export const normalizeBranches = (branches, boardLength) => {
  ...
  branches.forEach((branch, index) => {
    ...
    if (
      ...
      upperLaneIds.length === 0 ||
      lowerLaneIds.length === 0
    ) {
      return;
    }
    ...
    normalized.push({
      id:
        typeof branch?.id === "string"
          ? branch.id
          : `${createBranchId(start, end)}-${index}`,
      startId: start,
      endId: end,
...
export const createBranchBetweenSpaces = (board, branches, startId, endId) => {
  ...
  if (laneSet.endId - laneSet.startId < 3) {
    return {
      error: "分岐を作るには、開始地点と終了地点の間に最低2マス必要です。",
    };
  }
  ...
  if (hasOverlappingBranch(branches, laneSet.startId, laneSet.endId)) {
    return {
      error: "その区間にはすでに分岐があります。重なる分岐は作れません。",
    };
  }

  const originalPositions = [
    ...laneSet.upperLaneIds,
    ...laneSet.lowerLaneIds,
  ].map((id) => ({
    id,
    x: board[id].x,
    y: board[id].y,
...
export const buildBoardRouteSegments = (board, branches) => {
  ...
  sortedBranches.forEach((branch) => {
    if (cursor < branch.startId) {
      segments.push(
        board.slice(cursor, branch.startId + 1).map(({ x, y }) => ({ x, y })),
      );
    }
